...
export class ScreepsHttpClient extends EventEmitter {
   ...
   *
   * This is unlikely to work for anything but the official client;
   * `ticket` is either a hex-encoded Steam session ticket or encrypted app ticket
   * from the Steam client, generated by `getAuthSessionTicket` or `getEncryptedAppTicket`
   * in the Greenworks API.
   *
   * The server validates `ticket` either through the Steam Web API or by
   * decrypting it locally (Greenworks). Which method is used is advertised
   * as {@link Http.ScreepsVersionResponse.useNativeAuth} from {@link version}
   * and must match `useNativeAuth` here.
   *
   * Endpoint: `POST /api/auth/steam-ticket`
   * @param ticket Hex-encoded Steam session ticket or encrypted app ticket from the Steam client
   * @param useNativeAuth True to decrypt the ticket locally; false to authenticate via the
   *  Steam Web API. Defaults to false.
   * @category Endpoints: /auth
   */
  authSteamTicket(ticket: string, useNativeAuth = false): Promise<Http.AuthSteamTicketResponse> {
